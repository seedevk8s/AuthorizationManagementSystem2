...
<%@ include file="/WEB-INF/jsp/include/declare.jspf" %>/tags" %>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
    ...
    <c:url var="pageUrl" value="/secmgr/menu/popupMenuRole.do">
    	<c:param name="menuId" value="${hMap.menuId}"/>
...
<body>
    <div class="container-fluid">
	    ...
	    <div class="row col-sm-9 col-md-7">
			<form class="form-inline" method="GET" action='<c:url value="/secmgr/menu/popupMenuRole.do"/>'>
				...
				<input type="hidden" name="menuId" value="${hMap.menuId }"/>
		...
		<!-- 검색 끝 -->
	  	<form method="POST" action='<c:url value="/secmgr/menu/addRoles.do"/>'>
	  		<!-- 추가/닫기 시작 -->
			...
			<!-- 추가/닫기 끝 -->
	  		<table class="table table-condensed table-hover">
				...
				<tbody>
					<input type="hidden" name="menuId" value="${hMap.menuId }"/>
					<c:forEach var="addableMenuRoleVO" items="${addableMenuRoleVOList}">
						<tr>
							<td><input type="checkbox" name="roleId" value="${addableMenuRoleVO.roleId}"/></td>
							<td><c:out value="${addableMenuRoleVO.roleId}" /></td>
							<td><c:out value="${addableMenuRoleVO.roleName}" /></td>
							<td><c:out value="${addableMenuRoleVO.regUser}" /></td>
							<td><fmt:formatDate	value="${addableMenuRoleVO.regDateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
