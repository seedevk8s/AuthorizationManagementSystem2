...
<head>
    ...
    <c:url var="pageUrl" value="/secmgr/resource/popupResourceRole.do">
    	<c:param name="resId" value="${hMap.resId}"/>
...
<body>
    <div class="container-fluid">
	    ...
	    <div class="row col-sm-9 col-md-7">
			<form class="form-inline" method="GET" action='<c:url value="/secmgr/resource/popupResourceRole.do"/>'>
				...
				<input type="hidden" name="resId" value="${hMap.resId }"/>
		...
		<!-- 검색 끝 -->
	  	<form method="POST" action='<c:url value="/secmgr/resource/addRoles.do"/>'>
	  		<!-- 추가/닫기 시작 -->
			...
			<!-- 추가/닫기 끝 -->
	  		<table class="table table-condensed table-hover">
				...
				<tbody>
					<input type="hidden" name="resId" value="${hMap.resId }"/>
					<c:forEach var="addableResourceRoleVO" items="${addableResourceRoleVOList}">
						<tr>
							<td><input type="checkbox" name="roleId" value="${addableResourceRoleVO.roleId}"/></td>
							<td><c:out value="${addableResourceRoleVO.roleId}" /></td>
							<td><c:out value="${addableResourceRoleVO.roleName}" /></td>
							<td><c:out value="${addableResourceRoleVO.regUser}" /></td>
							<td><fmt:formatDate	value="${addableResourceRoleVO.regDateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
