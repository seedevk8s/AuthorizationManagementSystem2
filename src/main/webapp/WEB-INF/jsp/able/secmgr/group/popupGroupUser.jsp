...
<head>
    ...
    <c:url var="pageUrl" value="/secmgr/group/popupGroupUser.do">
...
<body>
    <div class="container-fluid">
    	<!-- 유저 추가 시작 -->
	    <h3 class="modal-title">유저 추가</h3>
	    <!-- 검색 시작 -->
	    <div class="row col-sm-9 col-md-7">
			<form class="form-inline" method="GET" action='<c:url value="/secmgr/group/popupGroupUser.do"/>'>
		...
		<!-- 검색 끝 -->
	  	<form method="POST" action='<c:url value="/secmgr/group/addUsers.do"/>'>
	  		<!-- 추가/닫기 시작 -->
			...
			<!-- 추가/닫기 끝 -->
	  		<table class="table table-condensed table-hover">
				...
				<tbody>
					<input type="hidden" name="groupId" value="${hMap.groupId }"/>
					<c:forEach var="addableGroupUserVO" items="${addableGroupUserVOList}">
						<tr>
							<td><input type="checkbox" name="userId" value="${addableGroupUserVO.userId}"/></td>
							<td><c:out value="${addableGroupUserVO.userId}" /></td>
							<td><c:out value="${addableGroupUserVO.userName}" /></td>
							<td><c:out value="${addableGroupUserVO.regUser}" /></td>
							<td><fmt:formatDate	value="${addableGroupUserVO.regDateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
