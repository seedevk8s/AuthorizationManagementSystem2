...
<head>
    ...
    <c:url var="pageUrl" value="/secmgr/user/popupUserGroup.do">
    	<c:param name="userId" value="${hMap.userId}"/>
...
<body>
    <div class="container-fluid">
	    ...
	    <div class="row col-sm-9 col-md-7">
			<form class="form-inline" method="GET" action='<c:url value="/secmgr/user/popupUserGroup.do"/>'>
				...
				<input type="hidden" name="userId" value="${hMap.userId }"/>
		...
		<!-- 검색 끝 -->
	  	<form method="POST" action='<c:url value="/secmgr/user/addGroups.do"/>'>
	  		<!-- 추가/닫기 시작 -->
			...
			<!-- 추가/닫기 끝 -->
	  		<table class="table table-condensed table-hover">
				...
				<tbody>
					<input type="hidden" name="userId" value="${hMap.userId }"/>
					<c:forEach var="addableUserGroupVO" items="${addableUserGroupVOList}">
						<tr>
							<td><input type="checkbox" name="groupId" value="${addableUserGroupVO.groupId}"/></td>
							<td><c:out value="${addableUserGroupVO.groupId}" /></td>
							<td><c:out value="${addableUserGroupVO.groupName}" /></td>
							<td><c:out value="${addableUserGroupVO.regUser}" /></td>
							<td><fmt:formatDate	value="${addableUserGroupVO.regDateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
